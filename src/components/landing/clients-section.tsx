...
export function ClientsSection() {
  return (
    <section id="clients" className="overflow-hidden bg-[#F9F8F4] py-8 relative z-40 w-full">
      ...
      <div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)] group">
        ...
        >
          <ul className="flex items-center justify-center md:justify-start [&_li]:mx-4 md:[&_li]:mx-8 [&_img]:max-w-none text-black">
            {[...clients, ...clients].map((client, idx) => (
              <li key={idx} className="w-[160px] md:w-[200px] h-[100px] md:h-[120px] flex items-center justify-center rounded-2xl p-4 transition-colors">
                <Image
                  src={client.logo}
                  alt={client.name}
                  width={160}
                  height={80}
                  className="object-contain w-auto h-auto max-h-[60px] md:max-h-[80px] brightness-0 opacity-70 hover:opacity-100 transition-opacity"
                />
              </li>
            ))}
          </ul>
